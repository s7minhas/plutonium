...
####
# see here for more details on data generation:
# https://github.com/s7minhas/plutonium/tree/main/code/03_downstreamAnalyses/data_us_commit
dat = read_dta( paste0(dpth, 'main_constraint_1.dta'))
dat = data.frame(dat, stringsAsFactors=F)

# load factor analysis summaries
screeDat = read.csv( paste0(dpth, 'factResults.csv'), stringsAsFactors=FALSE)
####
...
ggsave(screeViz,
  file=paste0(gpth, 'screeViz.pdf'),
  width=8, height=3, device=cairo_pdf )
ggsave(screeViz,
  file=paste0(gpth, 'screeViz.png'),
...
ggsave(facViz,
  file=paste0(gpth, 'facViz.pdf'),
  width=8, height=4, device=cairo_pdf )
ggsave(facViz,
  file=paste0(gpth, 'facViz.png'),
