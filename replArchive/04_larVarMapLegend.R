...
####
# Check if mapCol.rda already exists
if(!file.exists(paste0(gpth, 'mapCol.rda'))){
	# geo colors for nodes
	cmap = wmap = cshp(date=as.Date('2016-1-1'))
	wmap$cowc = countrycode(wmap$COWCODE, 'cown', 'cowc', warn=FALSE)
	...
	####
	# Generate legend map
	cmap@data$cowc = countrycode(cmap@data$COWCODE, 'cown', 'cowc', warn=FALSE)
	...
	ggsave(ggMap, file=paste0(gpth, 'mapLeg.png'))
	save(mapCol, ccols, cntryKey, file=paste0(gpth, 'mapCol.rda'))
} else {
	# Load existing mapCol.rda
...
loadPkg(c('grid', 'png'))
mapForCirc = rasterGrob(readPNG(paste0(gpth, 'mapLeg.png')), interpolate=TRUE)
####
