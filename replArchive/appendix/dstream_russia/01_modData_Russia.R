...
if(!file.exists(paste0(dpth, "modData_russia.rda"))){
...
usLevel = read.csv(paste0(dpth, 'main1_final.csv'))
...
if(!file.exists(paste0(dpth, 'geoInfo.rda'))){
  ...
  geoCent = distlist( as.Date('2019-12-31'),
    type = 'centdist' )
  save(geoMin, geoCap, geoCent, file=paste0(dpth, 'geoInfo.rda'))
} else { load(paste0(dpth, 'geoInfo.rda')) }
...
natSec = read.csv(paste0(dpth, 'natural_security_index.csv'), stringsAsFactors=F)
...
####
save(modData, file = paste0(dpth, "modData_russia.rda"))
