...
mf1 = lmer(
  econDelta_lfm ~
    USf1.l1 + GDP.l1 + polity.l1 + pop.l1 +
    beijDist + (1|idPt4), data = modData)
mf2 = lmer(
  econDelta_lfm ~
    USf2.l1 + GDP.l1 + polity.l1 + pop.l1 +
    beijDist + (1|idPt4), data = modData)
mf1HetEff = lmer(
  econDelta_lfm ~
    GDP.l1 + polity.l1 + pop.l1 +
    beijDist + (1 + USf1.l1 |idPt4), data = modData)
mf2HetEff = lmer(
  econDelta_lfm ~
    GDP.l1 + polity.l1 + pop.l1 +
    beijDist + (1 + USf2.l1 |idPt4), data = modData)
####
...
bf1HetEff = stan_lmer(
  econDelta_lfm ~
    GDP.l1 + polity.l1 + pop.l1 +
    beijDist + (1 + USf1.l1 |idPt4),
    data = modData, seed=6886)
bf2HetEff = stan_lmer(
  econDelta_lfm ~
    GDP.l1 + polity.l1 + pop.l1 +
    beijDist + (1 + USf2.l1 |idPt4),
    data = modData, seed=6886)
####
...
save(
  ...
  modData,
  file=paste0(rpth, 'dstreamModels_idPt4.rda')
)
####
