...
A simple latent variable model of the above variables produced three features, representing active US conflicts (F1), US defense spending / commitments (F2), and the economic shocks (F3). A PCA with three latent variables explained 89\% of the variance in the raw data.\footnote{A possible fourth factor, which we did not include, focuses on variance from GDP growth and changes in the stock market.}
