...
To populate these categories, we rely on sixteen variables distributed across the following broader categories:
...
The main source of constraint that is incomplete in the above framework is domestic political crises (i.e., apart from a response to economic shocks, which is captured).\footnote{Prior work has shown that political crises of this sort are rare / not likely consequential \citep{frieden:etal:2017} and we leave the inclusion of political measures to future work.} A simple latent variable model of the above variables produced two features, representing active US conflicts ($F_{1}$) and US defense spending / commitments ($F_{2}$).\footnote{In the appendix, we have included a visualization of how each of our component variables loaded into the factors we focus on in this paper.} Figure~\ref{fig:screeViz} illustrates the cumulative proportion of variance accounted for by each of the factors in the top panel, and in the bottom the variance of each factor. A PCA with two latent variables explained over 75\% of the variance in the raw data.\footnote{A possible third factor, which we did not include has as its most component employment rate but the interpretation of that factor as representing economic difficulties faced by the US is not at all clear given the other factor loadings.}
...
\begin{figure}[ht]
    ...
    \caption{Visualization of $F_{1}$, $F_{2}$, and $F_{3}$ over time.}
    \label{fig:facViz}
\end{figure}
\FloatBarrier
